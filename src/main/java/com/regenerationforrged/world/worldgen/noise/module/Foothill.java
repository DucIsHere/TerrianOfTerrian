 ...
 */
public record RadiusFoothillNoise(Noise mountainNoise, float startRadius, float transitionWidth) implements Noise {
    public static final Codec<RadiusFoothillNoise> CODEC = RecordCodecBuilder.create(instance -> instance.group(
        Noise.CODEC.fieldOf("mountain_noise").forGetter(RadiusFoothillNoise::mountainNoise),
        Codec.FLOAT.fieldOf("start_radius").forGetter(RadiusFoothillNoise::startRadius), // Khoảng cách bắt đầu dốc
        Codec.FLOAT.fieldOf("transition_width").forGetter(RadiusFoothillNoise::transitionWidth) // Độ rộng dải thoải (250)
    ).apply(instance, RadiusFoothillNoise::new));
